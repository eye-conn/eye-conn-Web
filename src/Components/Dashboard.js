...
import { Alert, Button, Card, CardHeader, IconButton, Link, Slide, Snackbar, Tab, Tabs, TextField, Typography } from '@mui/material';
...
function Dashboard({setAuth: hasAuth, setAuthLoading: hasAuthLoading, Socket: socket, ...props}) {
  ...
  const handleChangeEncrypt = event => {
    ...
    if (fileUploaded.type.slice(0,5) !== "image" || fileUploaded.size>104857600){
  ...
  const handleChangeDecrypt = async event => {
    ...
    let fileUploaded = event.target.files[0];
    if (!fileUploaded) return;
    if (fileUploaded.type.slice(0,5) !== "image" || fileUploaded.size>10485760){
      return alert('Please upload a image less than 10MB')
    }
    // console.log(fileUploaded.size)
    if (fileUploaded.size >= 307200){
  ...
  const config = {
    onUploadProgress: e => setProgressE(Math.ceil(parseInt(e.loaded)/parseInt(e.total) *100)),
...
.post(`${process.env.REACT_APP_HOST}/encrypt`,form,{...config},
  ...
  // console.log(updateData)
  const config = {
    onUploadProgress: e => setProgressD(Math.ceil(parseInt(e.loaded)/parseInt(e.total) *100)),
    cancelToken: cancelTokenSource.token
}
  axios
    .post(`${process.env.REACT_APP_HOST}/decrypt`, 
      form,config
    )
    .then((response) => {
      console.log(response.data);
          ...
          <Box sx={{ mt: 1,height: 330 }}>
            ...
            <Grid container spacing={3}>
              <Grid item xs={12} md={6}>
                <Box  sx={{ mt: 0, display: 'flex',flexDirection: 'column' ,justifyContent: 'center', height: 300, gap: 3 }}>
                  {/* <Typography variant="h6"  component="div">
                    Upload File
                  </Typography> */}
                  <form onSubmit={handleESubmit} style={{display: 'flex', flexDirection: 'row', gap: 15, alignSelf: 'center'}}>
                  <input
                    accept="image/*"
                    style={{ display: 'none' }} 
...
<CircularProgressWithLabel size={24}   value={progressE} variant='determinate' />   </>
):<Save fontSize='20' />}
variant="contained">Encrypt</Button>
                  </form>
                  {/* create a download button */}
                  { encryptImage &&
                   <Link href={encryptImage} download sx={{mt: 0, display: 'flex',flexDirection: 'column' , alignSelf:'center', width: 150}}>
                   <Button color="error" variant="contained" component="span" startIcon={<CloudDownload />}>
                    Download
                  </Button> 
                  </Link> }
              ...
              <Grid item xs={12} md={6}>
                <Box sx={{ mt: 0}}>
                  ...
                  <Box sx={{ mt: 3 }}>
                    <img src={EncryptPhoto
                      ? EncryptPhoto.path
                      : 'https://via.placeholder.com/150'} alt="preview" height={250}/>
                  </Box>
                </Box>
              </Grid>
            </Grid>
...
<CircularProgressWithLabel size={24} value={progressD} />
):<Save fontSize='20' />}
variant="contained">Decrypt</Button>
          ...
          <Snackbar open={msgOpen} autoHideDuration={3000} onClose={()=>{setMsgOpen(false)}} >
